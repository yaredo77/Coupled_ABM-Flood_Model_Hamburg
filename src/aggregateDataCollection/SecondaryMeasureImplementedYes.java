...
/**
 * This class records the cumulative number of households that implemented a secondary measure (evaluated at each time step).
 * 
 */
public class SecondaryMeasureImplementedYes implements AggregateDataSource{
	@Override
	public String getId() {
		return "Secondary measure implemented Yes (cum)";
	}
	...
	public Object get(Iterable<?> objs, int size) {
		int count = 0;
		for (Object o: objs) {
			if (!((House) o).getSecondaryMeasure().equals(GlobalVariables.getNoMeasure())) {
				count++;
			}
		}
		return count;
