...
/**
 * This class records the structural damage at a time step. To save time, compute only when there is a flood event.
 */
public class TotalBuildingDamage implements AggregateDataSource{
	@Override
	public String getId() {
		return "Building damage (per tick)";
	}
	...
	public Object get(Iterable<?> objs, int size) {
		...
		if (tick > 0 && Flood.getEventTimeSeries().get(tick-1) > 0) {
			for (Object o: objs) {
				totalbuildingDamage += ((House) o).getBuildingDamage();
			}
		}
		return totalbuildingDamage;
	}
