...
/**
 * This class records the content damage saved at a time step. To save time, compute only when there is a flood event.
 */
public class TotalContentDamageReduced implements AggregateDataSource{
	@Override
	public String getId() {
		return "Content damage saved (per tick)";
	}
	...
	public Object get(Iterable<?> objs, int size) {
		double totalContentDamageSaved = 0;
		int tick = (int)RunEnvironment.getInstance().getCurrentSchedule().getTickCount();
		if (tick > 0 && Flood.getEventTimeSeries().get(tick-1) > 0) {
			for (Object o: objs) {
				totalContentDamageSaved += ((House) o).getContentDamageReduced();
			}
		}
		return totalContentDamageSaved;
	}
