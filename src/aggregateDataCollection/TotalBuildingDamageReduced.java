...
/**
 * This class records the building damage saved at a time step. To save time, compute only when there is a flood event.
 */
public class TotalBuildingDamageReduced implements AggregateDataSource{
	@Override
	public String getId() {
		return "Building damage saved (per tick)";
	}
	...
	public Object get(Iterable<?> objs, int size) {
		double totalbuildingDamageSaved = 0;
		int tick = (int)RunEnvironment.getInstance().getCurrentSchedule().getTickCount();
		if (tick > 0 && Flood.getEventTimeSeries().get(tick-1) > 0) {
			for (Object o: objs) {
				totalbuildingDamageSaved += ((House) o).getBuildingDamageReduced();
			}
		}
		return totalbuildingDamageSaved;
	}
